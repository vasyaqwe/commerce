...
export const ComboboxProvider = ({
   ...
   const [onInputValueChange, setOnInputValueChange] = useState<
      ((v: string) => void) | null
   >(null)
   ...
   const shouldFilterItems =
      filtering === "strict" || (filtering === "auto" && isTyping)
   ...
   const [selectedItems, setSelectedItems] = useState<ComboboxItem[]>(
      comboboxValue
         ? [...itemsMap.values()].filter((item) =>
              (comboboxValue as string[]).includes(item.value),
           )
         : [],
   )
   ...
   useEffect(() => {
      setFilteredItems(
         shouldFilterItems
            ? getFilteredItemsMap(itemsMap, inputValue)
            : itemsMap,
      )
   }, [inputValue, itemsMap])
   ...
   return (
      <ComboboxContext.Provider
         value={{
            ...
            itemsMap,
            filteredItemsMap,
            highlightedItem: getElementByIndex(
               filteredItemsMap,
