...
export const multipleSelectionReducer = ({
   ...
   const reducer: UseComboboxProps<ComboboxItem>["stateReducer"] = (
      _,
      { changes, type },
   ) => {
      const isFocusInsideTriggerArea = triggerAreaRef.current?.contains?.(
      ...
      switch (type) {
         ...
         case useCombobox.stateChangeTypes.ItemClick: {
            ...
            if (changes.selectedItem != null) {
               ...
               const highlightedIndex = getIndexByKey(
                  items,
               ...
               const isAlreadySelected = multiselect.selectedItems.some(
                  (selectedItem) =>
                     selectedItem.value === changes.selectedItem?.value,
               )
         ...
         case useCombobox.stateChangeTypes.InputChange:
            return {
               ...changes,
               selectedItem:
                  changes.highlightedIndex === -1 ? null : changes.selectedItem,
            }
